...
- **Support Contact:** adam@calendarcolorkit.com
- **Website:** https://portal.calendarcolorkit.com
...
- ColorKit extension uses **`calendar.readonly`** scope for optional calendar colors feature
- We access **calendar list and colors only** (to enable per-calendar color defaults)
- We do **NOT** access event titles, descriptions, attendees, locations, or attachments
- We do **NOT** access event content or personal information in events
- All calendar customization (colors, time blocks) happens **locally in your browser**
...
- Calendar event titles, descriptions, attendees, or locations
- Event attachments, notes, or meeting links
...
**Note:** We do access calendar list names and colors via `calendar.readonly` scope, but this is only used locally to enable per-calendar color defaults. This data is not transmitted to our servers.
...
### 4. Google OAuth (Authentication & Calendar API)

**Service:** "Sign in with Google" authentication + Google Calendar API
**Location:** Google's global infrastructure
**Data Shared:** We request email, profile (name, picture), and calendar.readonly scope
**Purpose:** User authentication and reading calendar colors for event coloring
...
- We request **`calendar.readonly`** scope to read calendar list and colors
- This is **read-only** - we cannot modify, create, or delete calendar events
- We do **NOT** access event titles, descriptions, attendees, or content
...
- Users can **request data export** via email to adam@calendarcolorkit.com
...
- Contact adam@calendarcolorkit.com for corrections
...
- Contact adam@calendarcolorkit.com
...
- Security issues reported to: adam@calendarcolorkit.com
...
**What it does:** Enable Google OAuth authentication
**Use case:** Authenticate user for Google Calendar API access (calendar.readonly)
**Does NOT:** Access any Google data beyond calendar list and colors
...
#### 3. `https://portal.calendarcolorkit.com/*`
...
- Access event titles, descriptions, attendees, or locations
...
**Note:** We can read calendar list names and colors via `calendar.readonly` scope for the calendar colors feature. This is optional and used locally only.
...
- Feature preferences (day coloring, event coloring, time blocking settings)
- Event color mappings (custom colors you apply to events)
- Session tokens (for API authentication)
- Push subscription endpoint
- Calendar colors (cached from Google Calendar API)
...
- Updated to reflect actual `calendar.readonly` scope usage
- Removed outdated `cookies` and `notifications` permissions (removed from manifest)
- Added `identity` permission documentation
- Clarified calendar data access (list and colors only, not event content)
- Updated subscription model (freemium with 7-day trial, monthly/annual)
...
- We use `calendar.readonly` for calendar list and colors only (not event content)
- We do NOT read event titles, descriptions, attendees, or locations
- All color customizations are stored locally in the browser
...
- Contact adam@calendarcolorkit.com for refund requests
