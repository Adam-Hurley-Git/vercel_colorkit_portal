...
**Version:** 2.4 (Updated for Event Coloring)
**Extension Version:** 0.0.5
**Backend Version:** Next.js 15.5.9
...
- ✅ `identity` - Google OAuth for Calendar API integration (calendar.readonly)
...
2. **Custom Inline Colors** - Support for custom quick-access colors in event color picker via settings
...
```javascript
{
  ...
  "eventColoringEnabled": true,            // Boolean: Event coloring on/off
  // ... other feature flags
}
```
...
│     ↓                                                             │
│     Opens https://portal.calendarcolorkit.com/signup            │
│                                                                   │
...
- Extension: `Main Extension/background.js:272`
...
**Code Location:** `Main Extension/popup/popup.js:57-104`
...
1. **Vercel Environment Variable:**
   ...
   VAPID_SUBJECT=mailto:adam@calendarcolorkit.com
   ```
...
│   ├── featureRegistry.js    # Feature initialization
...
│   ├── event-coloring/       # Event coloring feature
...
```json
{
  ...
  "permissions": [
    "identity", // Google OAuth for Calendar API (calendar.readonly)
  ...
  "host_permissions": [
    ...
    "https://portal.calendarcolorkit.com/*" // Web app API
  ],
  ...
  "externally_connectable": {
    ...
    "matches": ["https://portal.calendarcolorkit.com/*"]
  },
...
async function init() {
  ...
  // Initialize toolbar, event coloring, etc.
}
...
curl -H "Authorization: Bearer <access_token>" \
  "https://portal.calendarcolorkit.com/api/extension/debug-push?test=true"
```
...
- `Main Extension/lib/subscription-validator.js` - Removed cache duration, getCachedStatus(), cacheStatus()
- `Main Extension/popup/popup.js` - Removed redundant API call, reads storage only
- `Main Extension/background.js` - Fixed push handler, removed lastChecked references
...
- `Main Extension/background.js` - Set flag on cancellation
- `Main Extension/popup/popup.js` - Dynamic routing based on user type
