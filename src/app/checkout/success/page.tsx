...
import { preparePaymentSuccessMessage, prepareAuthSuccessMessage } from '@/utils/extension-messaging';
...
export default async function SuccessPage() {
  ...
  // Prepare auth message with session tokens for extension
  // CRITICAL: This allows the extension to register its push subscription with the backend
  // Without this, push notifications won't work until user visits dashboard
  const authMessage = await prepareAuthSuccessMessage();

  // Prepare payment success message for extension
  const paymentMessage = await preparePaymentSuccessMessage();

  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-slate-50">
      {/* Send auth message with session tokens - enables push subscription registration */}
      {authMessage && <ExtensionNotifier message={authMessage} />}

      {/* Notify extension of payment success - unlocks pro features */}
      {paymentMessage && <ExtensionNotifier message={paymentMessage} />}
